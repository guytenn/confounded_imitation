...
def _import_random_agent():
    from src.rllib.contrib.random_agent.random_agent import RandomAgent
    return RandomAgent, RandomAgent._default_config


def _import_maddpg():
    from src.rllib.contrib import maddpg
    return maddpg.MADDPGTrainer, maddpg.DEFAULT_CONFIG


def _import_alphazero():
    from src.rllib.contrib.alpha_zero.core.alpha_zero_trainer import\
        AlphaZeroTrainer, DEFAULT_CONFIG
    return AlphaZeroTrainer, DEFAULT_CONFIG


def _import_bandit_lints():
    from src.rllib.contrib.bandits.agents.lin_ts import LinTSTrainer, TS_CONFIG
    return LinTSTrainer, TS_CONFIG


def _import_bandit_linucb():
    from src.rllib.contrib.bandits.agents.lin_ucb import LinUCBTrainer, \
        UCB_CONFIG
