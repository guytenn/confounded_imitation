...
from src.dril.a2c_ppo_acktr.distributions import Bernoulli, Categorical, DiagGaussian
...
class Policy(nn.Module):
    def __init__(self, obs_shape, action_space, base=None, base_kwargs=None, normalize=True):
        ...
        #     try:
        #         import mpi4py
        #         from src.stable_baselines import TRPO
        #     except ImportError:
        #         mpi4py = None
        #         DDPG, TRPO = None, None
        #
        #     from src.stable_baselines import PPO2
        #
    ...
    def get_action(self, inputs, deterministic=False):
        ...
        if self.action_space == "Discrete":
            return self.dist(actor_features).logits
...
class CNNBase(NNBase):
    def __init__(self, num_inputs, recurrent=False, hidden_size=512, normalize=True):
        ...
        with torch.no_grad():
            n_flatten = self.cnn(torch.zeros(1, num_inputs, 80, 80)).shape[1]

        self.fc1 = (nn.Linear(n_flatten, hidden_size))
        self.relu = nn.ReLU()
